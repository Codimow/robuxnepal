...
export = {
    ...
    async execute(interaction: any) {
        ...
        const hasModeratorRole = config.MODERATOR_ROLE_ID && config.MODERATOR_ROLE_ID !== 'MODERATOR_ROLE_ID' && 
        ...
        if (!hasModeratorRole) {
            const errorEmbed = new EmbedBuilder()
                ...
                .setDescription('Only moderators can verify payments! You need the moderator role to use this command.')
                .setTimestamp();
        ...
        await PaymentSheetService.updatePaymentStatus(orderId, status as 'incomplete' | 'complete');
        ...
        const verifyEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '👤 Discord User', value: `${paymentData.discordUsername} (${paymentData.discordUserId})`, inline: true },
                { name: '🎮 Roblox Username', value: paymentData.robloxUsername, inline: true },
                { name: '💰 Robux Amount', value: paymentData.robuxAmount.toString(), inline: true },
                { name: '💳 Payment Amount', value: `Rs.${paymentData.paymentAmount}`, inline: true },
        ...
        if (status === 'complete') {
            const successEmbed = new EmbedBuilder()
                ...
                .setDescription(`Thanks your order is completed.`)
                .addFields(
                    { name: '🆔 Order ID', value: paymentData.orderId, inline: true },
                    { name: '🎮 Roblox Username', value: paymentData.robloxUsername, inline: true },
                    { name: '💰 Robux Amount', value: paymentData.robuxAmount.toString(), inline: true },
